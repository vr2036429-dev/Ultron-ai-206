...
function generateOfflineResponse(userPrompt, userName = "Asik") {
  ...
  if (p.includes("screen") || p.includes("error fix") || p.includes("fix karo") || p.includes("karo fix") || p.includes("what is on my screen") || p.includes("read screen")) {
    return {
...
Keep it crisp, futuristic, and structured.`,
        ...
        const sources = webChunks.map((chunk) => ({
...
app.post("/api/vision", async (req, res) => {
  try {
    ...
    const ai = getGenAI();
    if (ai) {
      const systemInstruction = mode === "camera" ? "You are ULTRON Visual Perception Core. Analyze the camera stream: identify real-world objects, documents, screens, hardware components, printed text, and environment context." : "You are ULTRON Screen Intelligence Core. Inspect this UI screenshot thoroughly: identify buttons, input fields, navigation bars, modal dialogs, error messages, and guide the user on what action to take.";
      try {
        const response = await ai.models.generateContent({
          model: "gemini-3.8-flash",
          contents: [
...
${JSON.stringify(memoryContext)}`;
      ...
      for (const msg of historyList.slice(-8)) {
        ...
        const prev2 = formattedContents[formattedContents.length - 1];
        if (prev2 && prev2.role === role) {
          prev2.parts.push({ text });
        } else {
...
app.post("/api/coder/diagnose", async (req, res) => {
  try {
...
${recentHistory.map((m) => `${m.role}: ${m.content}`).join("\n")}` : ""}`;
              ...
              config: {
                ...
                speechConfig: {
                  voiceConfig: {
                    prebuiltVoiceConfig: {
                      voiceName: "Puck"
                      // Crisp, commanding, futuristic assistant voice
                    }
                  }
                },
                systemInstruction: {
              ...
              callbacks: {
                ...
                onmessage: (serverMessage) => {
                  ...
                  if (serverMessage.toolCall) {
                    ...
                    console.log("[ULTRON Live Server] Tool call invoked in Live session:", calls.map((c) => c.name));
          ...
          if (liveSession && isSessionActive && msg.callId) {
            try {
              ...
              const responseData = typeof msg.output === "object" && msg.output !== null ? msg.output : { output: msg.output || "success" };
